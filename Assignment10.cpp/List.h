...
template <class typeName>
int List<typeName>::ListLimit() const
{
	return _listLimit;
}
/**************************************************************************
...
string List<typeName>::OutputList() const
{
	...
	{//Begin While
		...
		while(traverse != NULL)
		{
			...
			out << "Data: " << traverse->GetData() << endl;
...
//	Node<typeName> * temp;
		//D E C L A R A T I O N S
		Node<typeName>*  createNew;   //CALC - used to create dynamic memory
	...
	{
		//Creates new dynamic memory
		createNew  = new Node<typeName>;

		//Sets the data of the newly created node
		createNew->SetData(data);
